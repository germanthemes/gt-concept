...
function gt_concept_customize_register_typography_settings( $wp_customize ) {
	...
	if ( ! class_exists( 'GermanThemes_Local_Fonts' ) ) {
		$wp_customize->add_control( new GT_Concept_Customize_Plugin_Control(
			$wp_customize, 'gt_local_fonts_plugin', array(
				'label'       => esc_html__( 'More Fonts', 'gt-concept' ),
				'description' => esc_html__( 'You can install the GT Local Fonts plugin to extend the typography options with additional local GDPR-compatible fonts.', 'gt-concept' ),
				'section'     => 'gt_concept_section_typography',
				'settings'    => array(),
				'priority'    => 80,
			)
